...
emotion_history = deque(maxlen=history_length)
ear_history = deque(maxlen=history_length)
EAR_THRESHOLD = 0.1  # Threshold for drowsy detection
...
while cap.isOpened():
    ...
    for face in faces:
        ...
        # Compute alertness
        drowsy_emotions = sum(1 for e in emotion_history if e in DROWSY_LABELS)
        avg_ear = np.mean(ear_history) if len(ear_history) > 0 else 1.0
